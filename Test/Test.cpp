...
float angle = 0.0f;            // угол вращения для OX1
float verticalOffset = 0.0f;   // вертикальное перемещение для OX2
float horizontalOffset = 0.0f; // оризонтальное перемещение для OX3
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS)
        verticalOffset -= 0.8f * deltaTime;
    if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
        verticalOffset += 0.8f * deltaTime;

    // перемещение OX3
    if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS)
        horizontalOffset -= 1.5f * deltaTime;
    if (glfwGetKey(window, GLFW_KEY_O) == GLFW_PRESS)
        horizontalOffset += 1.5f * deltaTime;

    // ограничения
    angle = glm::clamp(angle, -150.0f, 150.0f); // ограничение угла вращения для ОХ1
    verticalOffset = glm::clamp(verticalOffset, -0.08f, 0.5f);   // ограничение для OX2
    horizontalOffset = glm::clamp(horizontalOffset, -0.0f, 0.6f); // ограничение для OX3
}
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model.Draw(shaderProgram, angle, verticalOffset, horizontalOffset);
